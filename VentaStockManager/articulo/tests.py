...
class SignalDeleteSheetsTests(TestCase):
    ...
    @override_settings(SHEETS_DELETE_SYNC_ENABLED=False)
    @mock.patch('articulo.signals.log')
    def test_flag_off_no_encola(self, mock_log):
        # Por default el flag está apagado y no debería pasar nada.
        # Mockeamos async_task para detectar cualquier llamada y fallar.
        with mock.patch('django_q.tasks.async_task') as mock_async:
            articulo = self._crear_articulo(codigo_interno='OFF1')
            articulo.delete()
            mock_async.assert_not_called()

    @override_settings(SHEETS_DELETE_SYNC_ENABLED=True)
